...
TEST(LANCZOS_IR_EV, CalculateFromLaplaceMatrix) {
  ...
  for (int i = 0; i < (int)MatrixSize.size(); ++i) {
    Eigen::SparseMatrix<double> A =
        CreateLaplaceMatrix<Eigen::SparseMatrix<double>>(MatrixSize.at(i));
    std::vector<double> error =
        runLanczosN(A, 10, NumberOfTmpEigenvalues.at(i),
                    NumberOfEigenvalues.at(i), 1, 1e-8, false, print_result);
...
TEST(LANCZOS_IR_EV, CalculateFromRandomSparse) {
  ...
  for (int i = 0; i < (int)MatrixSize.size(); ++i) {
    Eigen::SparseMatrix<double> A =
        CreateRandomSparse(MatrixSize.at(i), NumberOfEntries.at(i), 42 * i);
    std::vector<double> error =
        runLanczosN(A, 10, NumberOfTmpEigenvalues.at(i),
                    NumberOfEigenvalues.at(i), 1, 1e-8, false, print_result);
    EXPECT_LE(*std::max_element(error.begin(), error.end()), 1e-10);
